...
A page can be generated by a script instead of markdown. Add a
`script:` field to its frontmatter; the file (relative to
`src/_scripts/`) is evaluated at build time and its string result
becomes the page body (raw HTML). The markdown body is ignored.

`script`   the script file under `src/_scripts/`, e.g. `hello.d`
`output`   a relative path inside the site root: the script result
           is written to that file instead of generating a page
           (e.g. `data.json`); the page is not rendered and does not
           appear in the navigation. Requires `script`; cannot be
           combined with `redirectAs`; duplicate paths are errors

Scripts run with the site context bound:

`site`     the site configuration: siteName, siteAuthor,
           siteDescription, siteLang, siteCopyright, baseUrl,
           siteGeneratedBy
`posts`    every post: title, date, tags, url, draft, text,
           description
`pages`    the custom pages: slug, url, title, redirectAs
...
page error; the output directory is left untouched.

#### Script language

A tiny Ruby-flavoured, dynamically typed, pure language. There is
one way to call a function: `f(a, b)`; `f` alone is just the
function value. Everything is an expression; a program, a `def`
body and a lambda body are sequences of adjacent expressions whose
value is the last one.

Literals: `42`, `1.5`, `"text #{expr}"` (interpolation, nested
strings allowed), `true`, `false`, `nil`, `[1, 2]`,
`{"a" => 1}`. Lambdas: `{ x, y -> expr ... }`. Definitions:
`def f(a, b) expr ... end` (top-level `def`s see each other in any
order). Conditionals: `if cond then expr else expr end` (the
`else` branch is optional). Operators: `+ - * / % == != < > <=
>= && || !`, unary minus. Only `false` and `nil` are falsy. There
are no loops and no assignment; use recursion and `map`/`filter`.

Built-ins:

    len(x)          length of a string, array or map
    at(x, i)        element of an array or string at index i
    get(m, k)       value for key k in a map (nil when missing)
    append(a, v)    copy of array a with v appended
    concat(a, b)    array or string concatenation
    join(a, sep)    array joined into a string
    split(s, sep)   string split into an array
    map(a, f)       array with f applied to each element
    filter(a, f)    array of elements where f is truthy
    sort(a)         sorted array of numbers or strings
    reverse(a)      reversed array
    first(a)        first element (nil when empty)
    last(a)         last element (nil when empty)
    keys(m)         keys of a map
    values(m)       values of a map
    contains(a, x)  substring or element check
    trim(s)         string without surrounding whitespace
    lower(s)        lowercase string
    upper(s)        uppercase string
    replace(s, f, t) string with occurrences of f replaced by t
    take(a, n)      first n elements (or characters)
