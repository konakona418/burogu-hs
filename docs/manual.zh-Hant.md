...
```
burogu image DIGEST 檔案 [--quality N] [--max-dim N] [--clipboard]
```
...
`--clipboard` 改從 wayland 剪貼簿讀圖（wl-paste；alt 文字為
              "screenshot"）
...
可以為文字標註語言字形，讓瀏覽器選用地區正確的字形（如「骨」在
簡/繁/日字形不同）：
...
`ja` 與 `zh-Hant`/`zh-TW` 的字型跟隨主題（`--font-ja`、`--font-hant`，
可用 `theme.fonts.ja`/`theme.fonts.hant` 覆蓋）。
