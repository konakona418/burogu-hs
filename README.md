...
Posts live in `src/_post/*.md` with YAML frontmatter (`title`, `date`, `tags`, `description`, `draft`); everything else under `src/` is copied verbatim. Site configuration in `config.yaml`.
